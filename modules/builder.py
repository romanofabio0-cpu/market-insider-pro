...
def build_index(assets: List[Dict], news: List[Dict], calendar: List[Dict], fng: Dict):
    ...
    <div class="fng-meter">
        ...
        <div class="fng-bar"><div class="fng-indicator" style="left: {fng['value']}%;"></div></div>
    </div>
    '''
    ...
    html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Market Insider Pro</title>{CSS_CORE}</head><body>{get_header('home')}
    <div class="container">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
            <h2 class="section-title" style="margin:0;">GLOBAL MARKETS PULSE ⚡</h2>
            <div style="font-size:0.8rem; color:#00C853;"><span style="height:8px;width:8px;background:#00C853;border-radius:50%;display:inline-block;animation:pulse 1s infinite;"></span> LIVE DATA</div>
        </div>
        <div class="grid">{grid_html}</div>
        <div class="split-layout">
...
def build_signals_page(assets: List[Dict]):
    # Filtra i mercati volatili (> 3% o < -3%)
    ...
    html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pro Signals Room</title>{CSS_CORE}</head><body>{get_header('signals')}
    <div class="container"><h2 class="section-title">🚨 PRO SIGNALS ROOM (ALGO SCREENER)</h2><p style="color:#888; margin-bottom:30px;">Real-time algorithmic screener detecting high volatility and institutional order blocks.</p><div class="panel"><table style="width:100%;"><thead><tr><th>ASSET</th><th>PRICE</th><th>24H VOLATILITY</th><th>ALGO SIGNAL</th><th style="text-align:right;">ACTION</th></tr></thead><tbody>{rows}</tbody></table></div></div>
    {MODALS_HTML} {get_footer()}</body></html>'''
...
def build_brokers_page():
    ...
    html_cards = ""
    for b in brokers:
        html_cards += f'<div class="broker-card"><div style="display:flex; align-items:center;"><div class="broker-logo">🏦</div><div class="broker-info"><h3 style="margin:0; color:#fff;">{b["name"]}</h3><div class="broker-tags"><span>{b["type"]}</span><span>{b["pros"]}</span></div></div></div><button class="btn-trade" style="padding:12px 24px;" onclick="openWaitlist()">{b["cta"]}</button></div>'
    
    html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Partner Brokers</title>{CSS_CORE}</head><body>{get_header('brokers')}
    <div class="container"><h2 class="section-title">💸 EXCLUSIVE BROKER OFFERS</h2><p style="color:#888; margin-bottom:30px;">Trade with the tools the pros use. Use our partner links below to claim exclusive sign-up bonuses and fee discounts.</p><div style="max-width:800px; margin:0 auto;">{html_cards}</div></div>
    {MODALS_HTML} {get_footer()}</body></html>'''
...
def build_academy():
    ...
    for _, m in ACADEMY_CONTENT.items():
        for l in m['lessons']:
            c_html = f'''<div style="filter: blur(6px); pointer-events: none;">{l['html']}</div><div style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%); text-align:center; background:#111; padding:40px; border:2px solid var(--accent); border-radius:12px; z-index:10;"><h2 style="color:#FFD700;">🔒 VIP CONTENT</h2><button class="btn-trade" onclick="openWaitlist()">GET VIP PASS</button></div>''' if l.get("vip") else l['html']
            html = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{l['title']}</title>{CSS_CORE}</head><body>{get_header('academy')}<div class="container"><div class="academy-grid"><div class="sidebar">{sidebar}</div><div class="lesson-content" style="position:relative;">{c_html}</div></div></div>{MODALS_HTML}{get_footer()}</body></html>'''
