...
from modules.news_engine import scarica_news
# Assicurati di importare le nuove build
...
def main():
    ...
    db_crypto = scarica_crypto_live()
    assets = genera_dataset_completo(db_crypto)
    fng_data = scarica_fear_greed() # SCARICA IL FEAR & GREED
    ...
    # 3. Costruzione Sito
    build_index(assets, news, calendar, fng_data) # Passato FNG alla home
    build_signals_page(assets) # NUOVA: Signals Room
    build_brokers_page()       # NUOVA: Hub Broker
    build_chart_pages(assets)
    build_academy()
    build_chat()
    try:
        build_wallet() # Mantieni il wallet
    except NameError:
